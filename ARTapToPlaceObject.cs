...
public class ARTapToPlaceObject : MonoBehaviour
{
    ...
    void Update()
    {
        //placementIndicator.SetActive(gameController.interactionEnabled);
        ...
        if(placementPoseIsValid && Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Began){
            ...
            if (!EventSystem.current.IsPointerOverGameObject(Input.GetTouch(0).fingerId)) // if the user has NOT touched a ui element
            {
                ...
                if (gameController.interactionEnabled)
                {
                    PlaceObject();
                }
                //}
    ...
    public void PlaceObject(){
        ...
        if(oldObject != null){
            ...
            if (oldObject.GetComponent<UVMapper>())
            {
                Destroy(oldObject.GetComponent<Renderer>().material.mainTexture);
            }
        ...
        Bounds objectBounds = GetChildRendererBounds(objectToPlace);

        oldObject = Instantiate(objectToPlace, parentContainer.transform);
        //oldObject.transform.localPosition = Vector3.zero + objectBounds.center;
